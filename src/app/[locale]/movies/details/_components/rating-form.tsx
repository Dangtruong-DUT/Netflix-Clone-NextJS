...
export default function RatingForm({ onSubmit }: Props) {
    ...
    return (
        <div
            className={`w-[70%] mb-4 transition-all duration-100 ${isExpanded ? 'bg-white/5 rounded-lg p-3 md:p-5 backdrop-blur-sm border-l-4 border-brand' : ''}`}
        >
            {!isExpanded ? (
                <button
                    onClick={() => setIsExpanded(true)}
                    className='w-full border-l-5 border-0 border-l-brand py-2 px-3 bg-[#ffffff]/15  hover:bg-[#ffffff]/30 cursor-pointer rounded-lg text-left transition-all duration-200 group'
                >
                    <span className='text-gray-100 text-base'>{t('addRating')}</span>
                </button>
            ) : (
                <form onSubmit={handleSubmit} className='space-y-3 md:space-y-4'>
                    <div className='flex items-center justify-between'>
                        <h3 className='text-base font-bold text-white'>{t('yourRating')}</h3>
                                ...
                                <div className='relative flex items-center gap-3 md:gap-4'>
                                    ...
                                    {(hoverRating > 0 || rating > 0) && (
                                        <div className='absolute left-full ml-3 md:ml-4'>
                                            <div className='px-1 py-1 rounded text-sm font-medium whitespace-nowrap border border-gray-600 shadow-sm'>
                                                {hoverRating > 0
                    ...
                    <div className='flex flex-col sm:flex-row gap-2 md:gap-3 pt-2 justify-center items-center max-w-xs mx-auto'>
                        <button
                            ...
                            className='w-[70px] h-[32px] sm:w-[80px] md:w-[90px] py-1 px-2 md:py-1 md:px-3 rounded-[4px] font-medium bg-[#6d6d6e]/70 hover:bg-[#6d6d6e]/40 transition-all duration-200 backdrop-blur-sm text-sm cursor-pointer'
                        >
                            {t('cancel')}
                        </button>
                        <button
                            type='submit'
                            disabled={rating === 0}
                            className={`w-fit-content h-[32px] sm:w-[80px] md:w-[90px] py-1 px-2 md:py-1 md:px-3 rounded-[4px] font-medium transition-all duration-200 text-sm ${
                                rating === 0
